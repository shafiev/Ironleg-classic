...
my $payment_id=param('payment_id');
...
#print "$privs_list";
  ...
  {
      if ($amount =~ /^\d+$/ && $date =~ /\d{4}-\d{2}-\d{2}/i && $school_id =~ /^\d+$/)
      {
	$query = "INSERT into $schema.$table(client_id,payment_date,amount)
		  VALUES('$school_id',to_date('$date','YYYY-MM-DD'),'$amount')";
	$redirect = "?action=add&school_id=$school_id&date=$date&amount=$amount";
	$redirect2 = "schools_info.pl?school_id=$school_id";
  ...
  elsif($action eq "edit")
  {
      if ($payment_id =~ /^\d+$/)
      {
      &add_edit_form($action,$payment_id)
      }
      else
      {
	print "Error";
  ...
  {
      if ($payment_id =~ /^\d+$/ && $amount =~ /^\d+$/ && $date =~ /\d{4}-\d{2}-\d{2}/i)
      {
	$query = "UPDATE $schema.$table
		  SET payment_date = to_date('$date','YYYY-MM-DD'),
		      amount = '$amount'
		  WHERE id='$payment_id'";
	$redirect = "?action=edit&payment_id=$payment_id";
	$redirect2 = "schools_info.pl?school_id=$school_id";
	&add_edit_del_db($query,$redirect,$redirect2);
      }
      else
  ...
  elsif($action eq "delete")
  {
    if ($payment_id =~ /^\d+$/)
    {
      $query = "DELETE from $schema.$table WHERE id='$payment_id'";
...
sub add_edit_form
{
  my($action,$payment_id) = @_;
    if ($action eq "edit")
    {
      my $sql0 = $db->prepare("SELECT to_char(payment_date,'YYYY-MM-DD'), amount
			       FROM $schema.$table
			       WHERE id='$payment_id'");
      $sql0->execute;
      ($date,$amount) = $sql0->fetchrow_array;        
    }
  print "<form name='select' method='get' onsubmit='return checkForm(this);'>";
  print "<table width='30%' border='1' cellspacing='1' cellpadding='1' align='left'>";
  print "<tr><td>Date: <td><input type='text' name='date' value='$date' size=13>";
  ...
  print "<tr><td width='50%'>Amount:</td><td><input type='text' name='amount' value='$amount'>";
  print "<tr><td>&nbsp;</td><td><input type='submit' value='Submit'>";
  $action .= "_db";
  print "<input type='hidden' name='action' value='$action'>";
  print "<input type='hidden' name='school_id' value='$school_id'>";
  print "<input type='hidden' name='payment_id' value='$payment_id'>";
  print "</table>";
  print "</form>";
  print "<script>
